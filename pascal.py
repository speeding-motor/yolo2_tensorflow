...
def get_train_labels(batch_box):
    ...
    for i, boxs in enumerate(batch_box):
        boxs = boxs[0: MAX_BOX_PER_PICTURE * 5]
        batch_boxs_format[i][0: len(boxs)] = boxs

    batch_box = np.reshape(batch_boxs_format, newshape=(batch_size, MAX_BOX_PER_PICTURE, 5))

    box_xy = (batch_box[..., 3:5] + batch_box[..., 1:3]) / 2
    box_wh = (batch_box[..., 3:5] - batch_box[..., 1:3])
    ...
    for i in range(batch_size):
        ...
        iou_value = iou.IOU().iou(boxs, anchor_boxs=ANCHORS)
        best_anchor = np.argmax(iou_value, axis=1)  # which anchor is the best anchor for the box
        grid_id = np.floor(box_xy[i][vaild_mask[i]] / config.IMAGE_HEIGHT * config.GRID_SIZE).astype(int)
